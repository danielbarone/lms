...
export const useStyles = makeStyles((theme) => ({
  root: props => ({
    '& > *': {
      color: props.status === 2 ? theme.ROW_TXT_PAID : props.datePriority === 1 ? theme.ROW_TXT_OVERDUE : theme.colors.text.primary,
    ...
    '&:hover': {
      backgroundColor: props.status === 0 ? props.datePriority === 1 ? theme.ROW_OVERDUE_HOVER : `${theme.colors.background.secondary} !important` : theme.ROW_PAID_HOVER,
    },
  }),
  detailsRoot: {
    '& > *': {
  ...
  invDetailsDescCont: props => ({
    paddingTop: '20px',
    paddingBottom: '20px',
    color: props.status === 2 ? theme.ROW_DESC_PAID : props.datePriority === 1 ? theme.ROW_DESC_OVERDUE : theme.colors.text.highlight,
  }),
  ...
  collapsedContainer: props => ({
    backgroundColor: props.status === 2 ? theme.DETAILS_PAID : props.datePriority === 1 ? theme.DETAILS_OVERDUE : theme.colors.background.secondary,
  }),
  detailsHeader: {
    marginBottom: '1em',
  },
  ...
  arrowIcons: props => ({
    color: props.status === 2 ? theme.EXPAND_PAID : props.datePriority === 1 ? theme.EXPAND_OVERDUE : theme.colors.text.highlight,
  }),
  expandBtn: {
    cursor: 'pointer'
  },
  ...
  sortArrow: {
    '& > svg': {
      color: `${theme.colors.text.highlight} !important`
    }
  },
  paginationComponent: {
    color: theme.colors.text.primary,
